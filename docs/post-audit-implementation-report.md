...
6. **Accessibility contrast failures** (found via Lighthouse, not previously audited): the footer copyright/privacy-policy text, the marquee strip text, and the language-switcher button text all measured below the 4.5:1 AA threshold (2.17–2.40:1 measured). A related **Label-in-Name (WCAG 2.5.3) mismatch**: the language-switcher button's `aria-label="Change language"` discarded its own visible text ("EN ▾"), so its accessible name didn't contain its visible label.
...
- **Contrast fixes** (WCAG 1.4.3, verified via Lighthouse before/after):
  - `.footer-copy` text: `rgba(138,134,128,0.5)`/`0.4`/`0.55` → `0.95` (footer copyright/privacy-policy text, plus the equivalent inline-styled blog-footer variant, plus `.journal-footer-copy`/`.vs-footer-copy`/`.wm-footer-copy` variants) — contrast raised from ~2.17–2.40:1 to ~5.0–5.5:1.
...
- **Re-verified, not modified**: mobile-menu `aria-expanded` sync, Escape-to-close + focus return, `inert` containment of background content, desktop "More" disclosure keyboard behavior, reduced-motion CSS/JS handling — all passed identically to the Phase 5 baseline.
...
- Lighthouse accessibility score on the homepage: **96 → 100** after the contrast and label fixes (local baseline; see §11).
...
- **`work-with-me.html`, `virtual-shoots.html`**: sticky-cta script include, WhatsApp prefill on the sticky bar, footer/lang-switch contrast fixes, hamburger touch-target fix.
- **`for-agencies.html`**: sticky-cta script include, footer/lang-switch contrast fixes, hamburger touch-target fix (WhatsApp prefill already present from an earlier phase).
...
Total: **81 files touched** (80 modified, 1 new) — reviewed in full via `git diff` before commit; no unrelated changes included.

## 14. Tests performed

- Chromium (Playwright, headless): all 10 required routes at 320×568, 360×800, 375×667, 375×812, 390×844, 430×932, 768×1024, 1024×768, 1280×800, 1440×900, 1920×1080 — zero unexpected horizontal overflow, zero console errors.
